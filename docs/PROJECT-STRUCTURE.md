...
5. **`backend/main.py`:** Exposes CORS-enabled FastAPI endpoints for `/health`, `/api/ingest`, `/api/search`, and `/api/chat`.
6. **`frontend/src/components/FileTree.jsx`:** Collapsible directory tree component displaying parsed repository files.
7. **`frontend/src/components/Chat.jsx`:** Interactive chat window displaying RAG responses and expandable source citations.
